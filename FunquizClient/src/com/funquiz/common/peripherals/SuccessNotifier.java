...
public class SuccessNotifier extends JFrame {
	/*
	 * SuccessNotifier method : used to initialize JPanel and required properties
	 * and add UI elements to the JFrame, JFrame will be used as a pop-up windows
	 * for success messages/confirmation messages
	 * 
	 * @param confirmMessage String is accepted to display the type of the
	 * success/confirmation message according to different situvations
	 */
...
//		setIconImage(new ImageIcon(getClass().getResource("/resources/logo-2.png")).getImage());
		...
		errorIconLabel.setIcon(new ImageIcon(SuccessNotifier.class.getResource("/com/funquiz/resources/success_icon.png")));
		...
		okButtonPanel.addMouseListener(new MouseAdapter() {
			/*
			 * Method mouseEntered to handle mouse click events SuccessNotifier JFrame
			...
			/*
			 * Method mouseExited to handle mouse click events SuccessNotifier JFrame
			...
			/*
			 * Method mouseClicked to handle mouse click events SuccessNotifier JFrame will
			 * be either disposed(closed) or redirected to a ContentPanel on mouse click
			 * 
			 * @param arg0 to get information about the mosue click
			 */
			@Override
			public void mouseClicked(MouseEvent arg0) {
				if (navigationRouter != null && contentRouter != null) {
					if (FunquizClient.authUser.getRole().equals("ADMIN")) {
						AdminGUI.selectedNavigation = navigationRouter;
						AdminGUI.selectedContent = contentRouter;
						AdminGUI.setSelectedPanel();
					} else {
						PlayerGUI.selectedNavigation = navigationRouter;
						PlayerGUI.selectedContent = contentRouter;
